...
class AccountRechargeRecordsPage extends StatefulWidget {
  const AccountRechargeRecordsPage({super.key});

  @override
  State<AccountRechargeRecordsPage> createState() =>
      _AccountRechargeRecordsPageState();
}

class _AccountRechargeRecordsPageState extends State<AccountRechargeRecordsPage> {
  final List<RechargeOrder> _items = <RechargeOrder>[];
  ...
  Future<void> _refresh() async {
    ...
    try {
      final List<RechargeOrder> list = await RechargeApi.getMyRechargeList(
        pageNum: _pageNum,
        pageSize: _pageSize,
      );
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(isZh ? '充值记录' : 'Recharge Records'),
        backgroundColor: const Color(0xFF0A1220),
        elevation: 0,
      ),
      body: Container(
  ...
  Widget _buildItem(RechargeOrder item, bool isZh) {
    final _StatusView status = _formatRechargeStatus(item.status, isZh);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          _kv(isZh ? '方式' : 'Method', item.rechargeMethod),
  ...
  _StatusView _formatRechargeStatus(int status, bool isZh) {
    switch (status) {
      case 1:
        return _StatusView(
          isZh ? '已通过' : 'Approved',
